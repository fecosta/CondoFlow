...
export default async function MoradorOcorrenciaDetailPage({ params }: { params: { id: string } }) {
  ...
  return (
    <div className="space-y-6 max-w-3xl">
      <div>
        <Link href="/morador/ocorrencias" className="inline-flex items-center text-sm text-gray-500 hover:text-gray-700 mb-3">
        ...
        <div className="flex flex-wrap gap-3 mt-2 text-sm text-gray-500">
          ...
          <span>·</span>
          <span>Aberta em {formatDateTime(ocorrencia.createdAt)}</span>
        </div>
      </div>
      ...
      <div className="bg-white border rounded-lg p-4 space-y-4">
        ...
        {ocorrencia.comentarios.length === 0 ? (
          ...
          <div className="space-y-3">
            {ocorrencia.comentarios.map((c) => (
              <div key={c.id} className={`rounded-lg p-3 text-sm ${c.user.id === session.user.id ? "bg-blue-50 ml-8" : "bg-gray-50 mr-8"}`}>
        ...
        {ocorrencia.status !== "FECHADA" && (
          <div className="pt-2 border-t">
            <ComentarioForm ocorrenciaId={ocorrencia.id} isSindico={false} />
          </div>
        )}
      </div>
    </div>
