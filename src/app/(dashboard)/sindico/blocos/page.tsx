...
export default async function SindcoBlocosPage() {
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500">{blocos.length} bloco(s) cadastrado(s)</p>
        </div>
        <NewBlocoDialog />
      </div>
            ...
            <Card key={b.id}>
              <CardContent className="pt-4 flex items-center justify-between gap-3">
                <div>
                  ...
                  <p className="text-xs text-gray-500 mt-0.5">{b._count.unidades} unidade(s)</p>
                </div>
                <div className="flex items-center gap-1">
                  <EditBlocoDialog blocoId={b.id} currentName={b.name} />
                  <DeleteBlocoButton blocoId={b.id} name={b.name} />
                </div>
              </CardContent>
            </Card>
          ))}
