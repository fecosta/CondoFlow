...
export default function NewAreaComumPage() {
  const [loading, setLoading] = useState(false);
  const router = useRouter();

  const { register, handleSubmit, formState: { errors } } = useForm<AreaComumInput>({
    resolver: zodResolver(areaComumSchema),
    defaultValues: { openTime: "08:00", closeTime: "22:00", maxDurationHours: 4, minAdvanceDays: 1, maxAdvanceDays: 30 },
  });
  ...
  async function onSubmit(data: AreaComumInput) {
    ...
    if (res.ok) {
      toast.success("Área comum criada!");
      router.push("/sindico/areas-comuns");
    } else {
      const body = await res.json().catch(() => ({}));
      toast.error(body.error ?? "Erro ao criar área.");
    }
  }
  ...
  return (
    <div className="max-w-xl space-y-4">
      <div>
        ...
        <h1 className="text-2xl font-bold mt-1">Nova Área Comum</h1>
      </div>
        ...
        <CardContent>
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            <div className="space-y-1">
              ...
              <Input id="name" placeholder="Ex: Salão de Festas" {...register("name")} />
              {errors.name && <p className="text-sm text-red-500">{errors.name.message}</p>}
            </div>

            <div className="space-y-1">
              <Label htmlFor="description">Descrição</Label>
              <Textarea id="description" placeholder="Regras, informações..." {...register("description")} />
            </div>
